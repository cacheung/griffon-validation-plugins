...
(async () => {
  ...
  const zipPath = R.pipe(
    R.filter(file => PACKAGE_NAME_REGEX.test(file)),
    R.head
  )(zipFiles);
  ...
  const { namespace } = R.pick(['namespace'], descriptor);
  ...
  const uuid = R.path(['data', field, 'uuid'], uploadedPlugin);
